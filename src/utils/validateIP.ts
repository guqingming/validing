import { isBooblean, isString, isHaveValue, isIP } from './validate'
import { getValueObject } from './function'
import validing from 'dist/validing'

const defaultConfig: validing.rules.IpConfig = {
  required: false,
  tip: ''
}

const validateIP = (config: validing.rules.IpConfig) => {
  try {
    if (config.required !== undefined && !isBooblean(config.required)) {
      throw Error('validateIp函数required参数必须为布尔值')
    }
    if (config.tip !== undefined && !isString(config.tip)) {
      throw Error('validateIp函数tip参数只能是字符串')
    }
  } catch (error) {
